...
ui <- fluidPage(theme = shinytheme("yeti"),
                ...
                sidebarLayout(
                  sidebarPanel(width=4,
                               titlePanel("SpotiData - sprawdŸ swoje statystyki s³uchania platformy Spotify", windowTitle = "SpotiData"),
                               hr(),
                               actionButton("zima", "Zima", width = "110px"),
                               actionButton("wiosna", "Wiosna", width = "110px"),
                               br(),
                               actionButton("lato", "Lato", width = "110px"),
                               actionButton("jesien", "Jesieñ", width = "110px"),
                               br(),
                               ...
                               #                separator = " do "),
                               hr(),
                               fileInput('files', 'Za³aduj swoje dane:', multiple = TRUE,
                                         accept = c("text/csv",
...
server <- function(input, output, session) {
  ...
  selected_spotidane <- reactiveValues(    #po prostu zbior wartosci reaktywnych - nie sugeruj sie nazwa
    ...
    arrow_index = 0,   #wylicza jaka wartosc nalezy dodaæ do '1:20' aby byli wyswieltani artysci z zakresu 1+array_index:20+array_index
    ...
    hover_read = FALSE,   # czy zczytywaæ dane z hovera - true gdy ma to sens
  ...
  observeEvent(input$click,{
    if(!selected_spotidane$click) {#w przypadku gdy wyswietlany jest 1. wykres
      ...
      #zapisanie wykonawcy jaki ma byc wyswietlany w drugim oknie - o ile click$y wystêpuje
      selected_spotidane$clicked[1] <-ifelse(!is.null(input$click$y), input$click$y, selected_spotidane$clicked[1])
    }
    else{#gdy drugi wykres jest wyswietlany, zbieramy klikniecie dla przycisku powróæ
      if(which(selected_spotidane$window)==1){
    ...
    output$distPlot <- renderPlot({
      if(!selected_spotidane$click){
        ...
        if(nrow(selected_spotidane[["x1"]])==0 || is.na(selected_spotidane[["x1"]][["artistName"]])) {#jesli nie mamy zadnych danych
          plot.new()
          text(0.5,0.5,"Wybrany zakres dat nie zwróci³ ¿adnych wyników dla danego pliku")
        }
        else{
          p <- ggplot(selected_spotidane[["x1"]], aes(x=reorder(artistName,-time), y=time)) +
            geom_bar(stat="identity", width=.7,fill="#1D428A")+
            theme_minimal()+
            # geom_text(aes(label = ifelse(time==selected_spotidane$max_value,paste0("Liczba przes³uchanych ró¿nych utworów wykonawcy: ", uniquesongs), uniquesongs)),
            #           size = 4.6, hjust = "top", nudge_y = -0.2, color = "white") +
            theme(axis.text.x = element_text(hjust = 1,size=11),
                  axis.text.y =element_text(size=15))+
            xlab(element_blank())+
            ylab("Liczba przes³uchanych godzin wykonawcy") +
            labs(title = "Ranking najd³u¿ej s³uchanych wykonawców") +
        ...
        if(which(selected_spotidane$window)==2){
          ...
          if(nrow(df)==0) {
            ...
            ggplot(df,aes(x=day,y=month,fill=month))+
              ...
              xlab('') +
              ylab('')+
              labs(title = paste0("Czêstoœæ s³uchania zespo³u ", selected_spotidane$selected, " z podzia³em na miesi¹ce"), caption = "• 2 •") +
        ...
        ##ten z lewej:
          ...
          if(nrow(y)==0 && nchar(y2)==0) {
            ...
            p <- ggplot(toplot, aes(x = factor(pora, dayparts), y = n, group = grupa, colour = factor(grupa, 1:2), fill = factor(grupa, 1:2))) +
              ...
              annotate("text", x = 1.5, y =  selected_spotidane$maxvalue*1.05, label = "bold(Powróæ)", parse = TRUE)+
              ...
              labs(title = paste0(selected_spotidane$selected, " - liczba odtworzeñ w ci¹gu tygodnia"),  caption = "1 • •") +
              ylab("Liczba odtworzeñ") +
              xlab(element_blank()) +
              scale_x_discrete(breaks = paste(weekdays(date("2020-01-20") + 0:6, abbreviate = TRUE), "12:00"),
                               limits = dayparts,
                               labels = weekdays(date("2020-01-20") + 0:6)) +
              scale_color_manual(values = c("red", "gray"), labels = c("Wybrany zakres dat", "Wszystkie dostêpne daty")) +
              scale_fill_manual(values = c("red", "gray"), labels = c("Wybrany zakres dat", "Wszystkie dostêpne daty"))
            ...
            if(readX>0 && length(readY)>0){
              ...
              if(2 %in% readY){
                ...
                p + annotate("rect", xmin = ifelse(readX>22, 22, readX), ymin = rectY,
                  ...
                  annotate("text", x = ifelse(readX>22, 22, readX)+3, y = rectY + selected_spotidane$maxvalue/12, label = paste("Najczêœciej s³uchany", "utwór w:",
                ...
                p + annotate("rect", xmin = ifelse(readX>22, 22, readX), ymin = rectY,
                  ...
                  annotate("text", x = ifelse(readX>22, 22, readX)+3, y = rectY + selected_spotidane$maxvalue/14, label = paste("Najczêœciej s³uchany", "utwór w:",
                                                                                                                                paste0(unlist(strsplit(as.character(toplot[readX, 1]), " "))[1],", ",
                                                                                                                                       unlist(strsplit(unlist(strsplit(as.character(toplot[readX, 1]), " "))[2], ":"))[1], "-",
                                                                                                                                       as.numeric(unlist(strsplit(unlist(strsplit(as.character(toplot[readX, 1]), " "))[2], ":"))[1]) + 6),
        ...
        ##poczatek prawego wykresu Jacy
          ...
          if(nrow(df)==0) {
            ...
            ggplot(df, aes(x=endTime, y=trackName, fill = trackName)) +
              ...
              xlab('') +
              theme_joy() +
              labs(title = "Czêstoœæ s³uchania utworów", caption = "• • 3") +
              theme(legend.position = 'none', axis.title.y = element_blank(),
  ...
  output$Opis <- renderUI({
    req(input$files)
    if(!selected_spotidane$click){
      HTML(paste("WSKAZÓWKA:", "U¿yj strza³ek na klawiaturze", "   - sprawdŸ co siê stanie!", sep="<br/>"))
    }
    else{
      if(selected_spotidane$window[1]){HTML(paste("WSKAZÓWKA:", "Aby dowiedzieæ siê wiêcej,", "najedŸ na punkt", sep="<br/>"))} #dla wykresu Paw³a
      else{HTML(paste("WSKAZÓWKA:", "U¿yj strza³ek na klawiaturze", "   - tym razem na boki!", sep="<br/>"))}
    }
  })
